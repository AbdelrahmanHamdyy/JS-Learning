...
const get3Countries = async function (c1, c2, c3) {
  try {
    ...
    ]); // Every array elements runs in parallel with the other
...
(async function () {
  ...
  console.log(res[0]); // The fastest promise to be settled whether fulfilled or rejected
});
...
// Returns all promises in an array regardless fulfilled or rejected
...
Promise.all([
  ...
  .then(res => console.log(res))
  .catch(err => console.error(err));
// A rejected promise short circuits all()
// Promise.any [ES2021]
Promise.any([
  Promise.resolve('Success'),
  Promise.reject('Error'),
  ...
  .then(res => console.log(res))
  .catch(err => console.error(err));
// Returns the first fulfilled promise and ignores rejected ones
